...
class ManagementBdd:
    def __init__(self):
        ...
        try:
            ...
            print(
                "Something went wrong, La connexion avec la base de données a échoué : {}".format(err))
            self.echec = 1
    ...
    def create_database(self):
        try:
            ...
            print("Requête SQL incorrecte :\n%s\nErreur détectée :\n%s"
                  % (self.requete, err))
            logging.error(
                "Error - class ManagementBdd, methode: create_database "+self.requete + ' - '+err)
            return 0

    def create_table_videos(self):
        try:
            logging.info(
                "class ManagementBdd, methode: create_table_videos - Start")
            var_cursor = self.cnx.cursor()
            ...
            logging.info(
                "class ManagementBdd, methode: create_table_videos - End")
            return 1

        except Exception as err:
            logging.error(
                "Error - class ManagementBdd, methode: create_table_videos "+self.requete + ' - '+err)
            # afficher la requête et le message d'erreur système :
            print("Requête SQL incorrecte :\n%s\nErreur détectée :\n%s"
                  % (self.requete, err))
            return 0
    ...
    def insert_donnes(self, donnes):
        ...
        try:
            ...
            sql_insert = (
                'INSERT INTO videos(titre, author,lien,anne_video,description,categorie) VALUES(%s,%s,%s,%s,%s,%s)')
            ...
            mon_cursor.execute(sql_insert, donnes)
            ...
            logging.error(
                "Error - class ManagementBdd, methode: insert_donnes "+self.requete + ' - '+err)
            return 0
